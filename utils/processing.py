...
import cv2
import numpy as np
# single image letter box and normalize
...
# batch images letter box and normalize
def batch_preprocess(imgs, input_shape, letter_box=True):
    concated_imgs = None
    for img in imgs:
        if letter_box:
            img_h, img_w, _ = img.shape
        ...
        if concated_imgs is None:
            concated_imgs = img
        else:
            concated_imgs = np.concatenate((concated_imgs, img), axis=0)
...
def postprocess(num_dets, det_boxes, det_scores, det_classes, img_w, img_h, input_shape, letter_box=True):
    ...
    boxes = det_boxes[0, :num_dets[0][0]] / np.array([input_shape[0], input_shape[1], input_shape[0], input_shape[1], 1], dtype=np.float32)
